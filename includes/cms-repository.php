...
function cms_default_services(): array
{
    return [
        ...
        ['Rental & Leasing Solutions', 'Residential and commercial rental services with verified tenants.', 'We help owners and tenants find practical rental matches with clear terms and professional coordination.', 'flaticon-set-building-plan', 'images/resource/service-details.jpg', 'page-service-details.php', 1, 30],
        ['Investment Consulting Services', 'Data-driven advice for high-return property investments.', 'We help investors understand location, demand, property type, and long-term value before making a move.', 'flaticon-set-investment-2', 'images/resource/service-d1.jpg', 'page-service-details.php', 1, 40],
    ];
}
...
{
    return [
        ['Highly professional and dedicated service', 'Uraca Realty made buying our first home in Davao a seamless experience. Marylyn was incredibly patient, showing us multiple properties that fit our criteria and explaining every document clearly. We could not be happier!', 'Emily Carter', 'First-time Homeowner', 'images/resource/testimonial1-1.jpg', 5, 1, 10],
        ['Expert market guidance and insights', 'Selling our property was stressful until we partnered with Uraca Realty. They provided clear pricing guidance, handled inquiries professionally, and secured a buyer much faster than we anticipated. Highly recommended!', 'Marcus Vance', 'Property Seller', 'images/resource/testimonial1-2.jpg', 5, 1, 20],
    ];
}
...
function cms_services(bool $enabledOnly = true): array
{
...
function cms_service(?int $id): ?array
{
    if ($id === null) {
        return null;
    }
    $stmt = db()->prepare('SELECT * FROM cms_services WHERE id = :id LIMIT 1');
    $stmt->execute(['id' => $id]);
    $service = $stmt->fetch();
    return $service ?: null;
}
...
{
    if ($id === null) {
        $stmt = db()->prepare('INSERT INTO cms_services (title, summary, body, icon_class, image_path, detail_url, is_enabled, sort_order)
            VALUES (:title, :summary, :body, :icon_class, :image_path, :detail_url, :is_enabled, :sort_order)');
    } else {
        $stmt = db()->prepare('UPDATE cms_services SET title = :title, summary = :summary, body = :body, icon_class = :icon_class,
...
function cms_testimonials(bool $enabledOnly = true): array
{
...
function cms_testimonial(?int $id): ?array
{
    if ($id === null) {
        return null;
    }
    $stmt = db()->prepare('SELECT * FROM cms_testimonials WHERE id = :id LIMIT 1');
    $stmt->execute(['id' => $id]);
